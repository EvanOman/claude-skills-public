...
| Domain-Driven Design | `/evan-skills:domain-driven-design` | Design a new system, or audit an existing one, against DDD and the patterns from *Architecture Patterns with Python*. A sizing gate scores what structure the system actually earns and forbids recommending anything above that tier; includes solo event storming, a read-only structural probe, and a tactical pattern catalog written cost-first. Advisory only — produces a design or an audit report, never code changes. |
